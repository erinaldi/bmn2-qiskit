...
# %%
def main(L: int = 2, N: int = 2, g2N: float = 0.2, optimizer: str = "COBYLA", varform: list = ['ry'], depth: int = 3, nrep: int = 1):
    ...
    print(f"Exact Result of discrete hamiltonian (qubit): {eigenvalues_qiskit(qubitOp)}")

    # Next, we create the variational form.
    var_form = EfficientSU2(qubitOp.num_qubits, su2_gates=varform, entanglement="full", reps=depth)
    ...
    optimizers = {"COBYLA": COBYLA(maxiter=5000),
    ...
    for i in np.arange(nrep):
        ...
        vqe = VQE(ansatz=var_form, optimizer=optim, initial_point=random_init, quantum_instance=q_instance, callback=store_intermediate_result)
    ...
    outfile = f"data/{optimizer}_{"-".join(varform)}_{nrep}.npy"
