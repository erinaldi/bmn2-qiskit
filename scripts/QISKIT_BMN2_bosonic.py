...
N = 4  # cutoff for Fock space. The number of qubits used will be K = log2(N)
annOp = np.array(np.diagflat(np.sqrt(np.linspace(1, N - 1, N - 1)), k=1))
with np.printoptions(
    precision=3, suppress=True, linewidth=120, threshold=sys.maxsize
...
def bosonHamiltonians(numBosons, bosonMatrixDim, coupling=0.1):
    ...
    annOP = np.array(
        np.diagflat(
            np.sqrt(np.linspace(1, bosonMatrixDim - 1, bosonMatrixDim - 1)), k=1
        )
    )
    ...
    for r in range(0, numBosons):
        x.append(
            (1 / np.sqrt(2)) * (bosonList[r] + np.transpose(np.conjugate(bosonList[r])))
        )
...
H2MM, H4MM1 = bosonHamiltonians(
    numBosons, bosonMatrixDim, coupling=0.1
)  # we can change the coupling by adding coupling=0.5 to the args
...
var_form = EfficientSU2(
    qubitOp.num_qubits, su2_gates=["ry"], entanglement="full", reps=3
)
...
optimizerList = [
    SLSQP(maxiter=600),
    COBYLA(maxiter=5000),
    L_BFGS_B(maxfun=5000, maxiter=20000),
    NELDER_MEAD(maxfev=5000),
]
...
for i in range(0, len(labelList)):
    ...
    plt.plot(
        countArr[i][100 : len(countArr[i]) - 1],
        valArr[i][100 : len(valArr[i]) - 1],
        label=labelList[i],
    )

plt.plot(
    countArr[-1],
    [np.real(energies[0])] * len(countArr[-1]),
    "y--",
    label="Exact Energy",
)
